...
int fts_readCmd(u8 *cmd, int cmdLength, u8 *outBuf, int byteToRead)
{
	...
	if (ret < 0) {
		log_error("%s fts_readCmd: ERROR %02X\n", tag, ERROR_I2C_R);
		return ERROR_I2C_R;
	}
...
int fts_writeCmd(u8 *cmd, int cmdLength)
{
	...
	if (ret < 0) {
		log_error("%s fts_writeCmd: ERROR %02X\n", tag, ERROR_I2C_W);
		return ERROR_I2C_W;
	}
...
int fts_writeFwCmd(u8 *cmd, int cmdLength)
{
	...
	if (ret < 0) {
		log_error("%s fts_writeFwCmd: ERROR %02X\n", tag, ERROR_I2C_W);
		return ERROR_I2C_W;
	}

	if (ret2 < OK) {
		log_error("%s fts_writeFwCmd: check echo ERROR %02X\n", tag, ret2);
...
int writeReadCmd(u8 *writeCmd1, int writeCmdLength, u8 *readCmd1, int readCmdLength, u8 *outBuf, int byteToRead)
{
	...
	if (ret < 0) {
		log_error("%s writeReadCmd: ERROR %02X\n", tag, ERROR_I2C_WR);
		return ERROR_I2C_WR;
	}
...
int readCmdU16(u8 cmd, u16 address, u8 *outBuf, int byteToRead, int hasDummyByte)
{
	...
	if (buff == NULL) {
		log_error("%s readCmdU16: ERROR %02X\n", tag, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
int writeCmdU16(u8 WriteCmd, u16 address, u8 *dataToWrite, int byteToWrite)
{
	...
	if (buff == NULL) {
		log_error("%s writeCmdU16: ERROR %02X\n", tag, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
int writeCmdU32(u8 writeCmd1, u8 writeCmd2, u32 address, u8 *dataToWrite, int byteToWrite)
{
	...
	if (buff2 == NULL) {
		log_error("%s writeCmdU32: ERROR %02X\n", tag, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
...
int writeReadCmdU32(u8 wCmd, u8 rCmd, u32 address, u8 *outBuf, int byteToRead, int hasDummyByte)
{
	...
	if (buff == NULL) {
		log_error("%s writereadCmd32: ERROR %02X\n", tag, ERROR_ALLOC);
		return ERROR_ALLOC;
	}
